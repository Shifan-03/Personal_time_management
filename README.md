            ...
            theme: {
                extend: {
                    ...
                    boxShadow: {
                        ...
                        'premium-dark': '0 10px 30px -10px rgba(0, 0, 0, 0.5), 0 1px 3px 0 rgba(255, 255, 255, 0.05)',
                    }
                }
            }
        }
...
<body class="bg-slate-50 dark:bg-darkBg text-slate-800 dark:text-slate-100 min-h-screen flex flex-col transition-colors duration-300">

    <!-- Notification Toast -->
    <div id="toast-container" class="fixed top-5 right-5 z-50 pointer-events-none space-y-3"></div>

    <!-- Header / Navbar -->
    <header class="border-b border-slate-200 dark:border-darkBorder bg-white dark:bg-darkSurface sticky top-0 z-30 px-6 py-4 flex items-center justify-between">
        <div class="flex items-center gap-3">
            <button onclick="toggleMobileNav()" class="md:hidden p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all">
                <i class="fa-solid fa-bars text-lg"></i>
            </button>
            <div class="flex items-center gap-2">
                <div class="h-9 w-9 bg-gradient-to-tr from-brand-500 to-indigo-600 rounded-xl flex items-center justify-center text-white shadow-md shadow-brand-500/20">
                    <i class="fa-solid fa-graduation-cap"></i>
                </div>
                <div>
                    <span class="font-black text-base tracking-tight block">STITCH</span>
                    <span class="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 tracking-wider -mt-1 block">StudyTrack Suite</span>
                </div>
            </div>
        </div>

        <div class="flex items-center gap-4">
            <button onclick="toggleTheme()" class="h-9 w-9 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700/60 border border-slate-200 dark:border-darkBorder rounded-xl flex items-center justify-center transition-all text-slate-600 dark:text-slate-300" title="Toggle Theme">
                <i id="theme-icon" class="fa-solid fa-sun text-base"></i>
            </button>
            <div class="h-8 w-[1px] bg-slate-200 dark:bg-darkBorder"></div>
            <div class="flex items-center gap-2.5">
                <div class="h-8 w-8 rounded-full bg-indigo-500/20 text-indigo-500 flex items-center justify-center font-bold text-xs border border-indigo-500/30">
                    SS
                </div>
                <div class="hidden sm:block">
                    <span class="text-xs font-bold block" id="header-user-name">Scholar Mode</span>
                    <span class="text-[9px] text-slate-400 block">Stitch Scholar</span>
                </div>
            </div>
        </div>
    </header>
    ...
    <div class="flex-grow flex flex-col md:flex-row relative">
        ...
        <nav id="sidebar-nav" class="hidden md:flex flex-col gap-1.5 p-4 flex-grow-0 w-full md:w-64 border-r border-slate-200 dark:border-darkBorder bg-white dark:bg-darkSurface">
            <span class="px-3 text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2">Core Hub</span>
            
            <a href="#" onclick="switchView('dashboard')" data-nav="dashboard" class="nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-brand-600 dark:hover:text-white transition-all font-medium text-sm">
                <i class="fa-solid fa-chart-pie w-5 text-center text-lg"></i>
                <span>Dashboard</span>
            </a>
            
            <a href="#" onclick="switchView('physics-overview')" data-nav="physics-overview" class="nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-brand-600 dark:hover:text-white transition-all font-medium text-sm">
                ...
                <span class="ml-auto text-[10px] bg-indigo-500/10 text-indigo-500 dark:text-indigo-400 px-2 py-0.5 rounded-full font-bold">Active</span>
            </a>

            <a href="#" onclick="switchView('calendar')" data-nav="calendar" class="nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-brand-600 dark:hover:text-white transition-all font-medium text-sm">
                <i class="fa-solid fa-calendar-days w-5 text-center text-lg"></i>
                <span>Exam Calendar</span>
                <span id="badge-calendar-count" class="ml-auto text-[10px] bg-rose-500 text-white px-2 py-0.5 rounded-full font-bold">3</span>
            </a>

            <a href="#" onclick="switchView('focus')" data-nav="focus" class="nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-brand-600 dark:hover:text-white transition-all font-medium text-sm">
                <i class="fa-solid fa-stopwatch w-5 text-center text-lg"></i>
                <span>Focus Timer</span>
            </a>

            <a href="#" onclick="switchView('analytics')" data-nav="analytics" class="nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-brand-600 dark:hover:text-white transition-all font-medium text-sm">
                <i class="fa-solid fa-square-poll-vertical w-5 text-center text-lg"></i>
                <span>Study Analytics</span>
            </a>

            <div class="h-[1px] bg-slate-200 dark:bg-darkBorder my-4"></div>
            <span class="px-3 text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2">Configure</span>

            <a href="#" onclick="switchView('settings')" data-nav="settings" class="nav-item flex items-center gap-3 px-3 py-3 rounded-xl text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-brand-600 dark:hover:text-white transition-all font-medium text-sm">
        ...
        <section class="flex-grow p-6 md:p-8 overflow-y-auto max-h-[calc(100vh-73px)] w-full">

            <!-- 1. HUB DASHBOARD VIEW -->
            <div id="view-dashboard" class="view-panel space-y-8">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-darkBorder pb-6">
                    <div>
                        <h2 class="text-2xl font-black dark:text-white" id="header-title">Workspace Hub Overview</h2>
                        <p class="text-sm text-slate-400">Track and synthesize your academic syllabus progression and focus objectives.</p>
                    </div>
                    <div class="flex gap-2">
                        <button onclick="openAddSubjectModal()" class="px-4 py-2.5 bg-brand-600 hover:bg-brand-700 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-brand-600/10">
                            <i class="fa-solid fa-plus mr-1.5"></i>Create Subject Track
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div class="bg-white dark:bg-darkSurface border border-slate-200 dark:border-darkBorder p-5 rounded-2xl shadow-premium dark:shadow-premium-dark flex items-center justify-between">
                        <div>
                            <span class="text-xs text-slate-400 font-bold uppercase tracking-wider">Total Progress Rate</span>
                            <h3 class="text-2xl font-black dark:text-white mt-1" id="stat-total-progress">62%</h3>
                            <p class="text-[10px] text-emerald-500 font-bold mt-1"><i class="fa-solid fa-chart-line mr-1"></i>+4.2% this week</p>
                        </div>
                    ...
                    <div class="bg-white dark:bg-darkSurface border border-slate-200 dark:border-darkBorder p-5 rounded-2xl shadow-premium dark:shadow-premium-dark flex items-center justify-between">
                        <div>
                            <span class="text-xs text-slate-400 font-bold uppercase tracking-wider">Study Time Logged</span>
                            <h3 class="text-2xl font-black dark:text-white mt-1" id="stat-total-hours">135.5 hrs</h3>
                            <p class="text-[10px] text-indigo-500 font-bold mt-1"><i class="fa-solid fa-stopwatch mr-1"></i>25 hrs focus goal</p>
                        </div>
                    ...
                    <div class="bg-white dark:bg-darkSurface border border-slate-200 dark:border-darkBorder p-5 rounded-2xl shadow-premium dark:shadow-premium-dark flex items-center justify-between">
                        <div>
                            <span class="text-xs text-slate-400 font-bold uppercase tracking-wider">Milestones Active</span>
                            <h3 class="text-2xl font-black dark:text-white mt-1" id="stat-exams-count">3 Exams</h3>
                            <p class="text-[10px] text-rose-500 font-bold mt-1" id="stat-next-exam">Next in 4 days</p>
                        </div>
                    ...
                    <div class="bg-white dark:bg-darkSurface border border-slate-200 dark:border-darkBorder p-5 rounded-2xl shadow-premium dark:shadow-premium-dark flex items-center justify-between">
                        <div>
                            <span class="text-xs text-slate-400 font-bold uppercase tracking-wider">Mastery GPA</span>
                            <h3 class="text-2xl font-black dark:text-white mt-1" id="stat-gpa">3.95</h3>
                            <p class="text-[10px] text-indigo-500 font-bold mt-1">Excellent performance</p>
                        </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div class="lg:col-span-2 bg-white dark:bg-darkSurface border border-slate-200 dark:border-darkBorder p-6 rounded-2xl shadow-premium dark:shadow-premium-dark">
                ...
                <div>
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="font-bold text-lg dark:text-white">Subject Mastery</h3>
                    ...
                    <div id="dashboard-subjects-container" class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div id="view-physics-overview" class="view-panel space-y-8 hidden">
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 dark:border-darkBorder pb-6">
                    <div>
                        <div class="flex items-center gap-2.5 mb-1.5">
                            <i class="fa-solid fa-atom text-indigo-500 text-xl" id="physics-header-icon"></i>
                    ...
                    <div class="flex gap-2">
                        <button onclick="switchView('focus')" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white text-xs font-bold rounded-xl transition-all" id="physics-action-start-btn"><i class="fa-solid fa-hourglass-start mr-1.5"></i>Start Focus Session</button>
                        <button onclick="switchView('dashboard')" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-darkBorder dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs font-bold rounded-xl transition-all"><i class="fa-solid fa-arrow-left mr-1.5"></i>Back</button>
                    </div>
                </div>

                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <!-- Progress Card -->
                    <div id="physics-progress-card-bg" class="bg-gradient-to-br from-indigo-900 to-slate-950 p-6 rounded-3xl text-white border border-indigo-500/30 flex flex-col justify-between shadow-xl transition-all">
                        <div>
                            <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="mt-8 space-y-4">
                            ...
                            <div class="flex justify-between text-xs text-indigo-300 font-semibold">
                    ...
                    <!-- Formulas / Quick Sandbox References Cheat Card -->
                    ...
                    <div class="bg-white dark:bg-darkSurface border border-slate-200 dark:border-darkBorder p-6 rounded-2xl shadow-premium dark:shadow-premium-dark flex flex-col justify-between">
                        <div>
                            ...
                            <div class="space-y-3 text-xs" id="physics-notes-list">
                                <!-- Dynamic notes will populate here -->
                            </div>
                        </div>
                        <div class="mt-4 flex gap-2">
                            <input id="physics-note-input" type="text" placeholder="Add study note..." class="flex-grow bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-darkBorder text-xs rounded-xl px-3 py-2 focus:outline-none focus:border-indigo-500 text-slate-800 dark:text-white">
                            <button onclick="addPhysicsNote()" class="p-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-xl text-xs font-semibold transition-all" id="physics-save-note-btn">Save</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <!-- Chapters List -->
                    <div class="lg:col-span-2 bg-white dark:bg-darkSurface border border-slate-200 dark:border-darkBorder p-6 rounded-2xl shadow-premium dark:shadow-premium-dark flex flex-col justify-between">
                        <div>
                            <div class="flex items-center justify-between mb-6">
                                <div>
            ...
            <div id="view-focus" class="view-panel space-y-8 hidden">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 bg-gradient-to-br from-slate-900 to-indigo-950 text-white p-8 rounded-3xl border border-indigo-500/20 shadow-xl flex flex-col justify-between relative overflow-hidden">
            ...
            <div id="view-settings" class="view-panel space-y-8 hidden">
                ...
                <div class="max-w-2xl bg-white dark:bg-darkSurface border border-slate-200 dark:border-darkBorder rounded-2xl shadow-premium dark:shadow-premium-dark divide-y divide-slate-100 dark:divide-darkBorder">
                    <div class="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <input id="settings-name-input" type="text" value="Scholar Mode" class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-darkBorder rounded-xl px-4 py-2 text-xs font-semibold focus:outline-none focus:border-brand-500 text-slate-800 dark:text-white w-full sm:w-64">
                    </div>
                    ...
                    <div class="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <select id="settings-synth-freq" class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-darkBorder rounded-xl px-4 py-2 text-xs font-bold focus:outline-none focus:border-brand-500 text-slate-800 dark:text-white w-full sm:w-64">
    ...
    <div id="add-subject-modal" class="fixed inset-0 z-50 overflow-y-auto hidden">
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-3xl bg-white dark:bg-darkSurface text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-slate-100 dark:border-darkBorder">
    ...
    <script>
        const APP_STATE = {
            ...
            exams: [
                { id: 'ex-1', subjectId: 'sub-physics', title: 'Physics Midterm: Kinematics', date: '2026-06-18', grade: 'A', day: 18, color: '#6366f1' },
                { id: 'ex-2', subjectId: 'sub-calculus', title: 'Vector Calc Midterm 2', date: '2026-06-22', grade: 'A-', day: 22, color: '#10b981' },
                { id: 'ex-3', subjectId: 'sub-cs', title: 'DSA Final Term Board', date: '2026-06-29', grade: 'A+', day: 29, color: '#f59e0b' }
            ]
        };
        ...
        function switchView(viewName) {
            ...
            document.querySelectorAll('.nav-item').forEach(item => {
                if (item.getAttribute('data-nav') === viewName) {
                    item.classList.add('bg-brand-50', 'dark:bg-indigo-500/10', 'text-brand-600', 'dark:text-white');
                } else {
                    item.classList.remove('bg-brand-50', 'dark:bg-indigo-500/10', 'text-brand-600', 'dark:text-white');
                }
            });

            const titles = {
                'dashboard': 'Workspace Hub Overview',
                'physics-overview': 'Syllabus Tracker Suite',
                'calendar': 'Exam Milestone Calendar',
                'focus': 'Focus Sandbox',
                'analytics': 'Study Analytics Hub',
                'settings': 'Preferences'
            };
            const headerTitleEl = document.getElementById('header-title');
            if (headerTitleEl) {
                headerTitleEl.innerText = titles[viewName] || 'Stitch Tracker';
            }
        ...
        function renderUpcomingExamsList() {
            ...
            sorted.slice(0, 3).forEach(ex => {
                ...
                container.innerHTML += `
                    <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-darkBorder">
                        <div class="flex items-center gap-3">
                            <div class="h-2 w-2 rounded-full" style="background-color: ${accentColor}"></div>
                            <div>
                                <h4 class="font-bold dark:text-white">${ex.title}</h4>
                                <span class="text-[10px] text-slate-400 font-semibold">${subName} — Target Grade: ${ex.grade}</span>
                            </div>
                        </div>
                        <div class="text-right">
                            <span class="text-[11px] font-black block text-rose-500">Day ${ex.day}</span>
        ...
        function renderSubjects() {
            ...
            if (APP_STATE.subjects.length === 0) {
                container.innerHTML = `
                    <div class="col-span-1 md:col-span-3 py-12 text-center text-slate-400 dark:text-slate-500 border-2 border-dashed border-slate-200 dark:border-darkBorder rounded-2xl">
                        <i class="fa-solid fa-folder-open text-3xl mb-3 block"></i>
                        <p class="text-sm font-semibold">No subjects active.</p>
                        <p class="text-xs mt-1">Click "Create Subject Track" to begin tracking your study syllabus.</p>
                    </div>
                `;
                return;
            }
            ...
            APP_STATE.subjects.forEach(subject => {
                ...
                const cardHTML = `
                    <div class="bg-white dark:bg-darkSurface border border-slate-200 dark:border-darkBorder p-6 rounded-2xl shadow-premium dark:shadow-premium-dark flex flex-col justify-between">
                        <div>
                            <div class="flex items-center justify-between mb-4">
                                <span class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">Current Mastery: ${subject.grade}</span>
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="h-2.5 w-2.5 rounded-full" style="background-color: ${subject.color}"></div>
                                </div>
                            </div>
                            <h4 class="font-extrabold text-lg dark:text-white">${subject.name}</h4>
                        ...
                        <div class="mt-8 space-y-4">
                            ...
                            <button onclick="viewSubjectTracker('${subject.id}')" class="w-full py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700/60 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-1.5">
                                <i class="fa-solid fa-folder-open text-xs"></i> View Syllabus
                            </button>
                        </div>
                    </div>
                `;
        ...
        function renderActiveSubjectDetails() {
            ...
            if (!activeSub) {
                ...
                document.getElementById('physics-chapters-list').innerHTML = `
                    <div class="py-12 text-center text-slate-400 dark:text-slate-500">
                        ...
                        <p class="text-sm font-semibold">Please create a track from your dashboard workspace hub.</p>
                    </div>
                `;
                return;
            }
            ...
            const startFocusBtn = document.getElementById('physics-action-start-btn');
            if (startFocusBtn) startFocusBtn.style.backgroundColor = activeSub.color;
        ...
        function renderPhysicsNotes() {
            ...
            if (!activeSub.recentNotes || activeSub.recentNotes.length === 0) {
                container.innerHTML = '<p class="text-[11px] text-slate-400 italic">No notes logged. Enter updates below to tracking derivations or formulae.</p>';
                return;
            }
            ...
            activeSub.recentNotes.forEach(n => {
                container.innerHTML += `
                    <div class="flex items-start gap-2.5 text-slate-600 dark:text-slate-300">
                        <i class="fa-solid fa-circle-arrow-right text-[10px] mt-1" style="color: ${activeSub.color}"></i>
                        <div>
                            <span class="font-bold text-[10px] block" style="color: ${activeSub.color}">${n.date}</span>
        ...
        function addPhysicsNote() {
            ...
            input.value = '';
            renderPhysicsNotes();
            showNotification('Saved study tracker note.');
        }
        ...
        function renderPhysicsChapters() {
            ...
            if (!activeSub.chapters || activeSub.chapters.length === 0) {
                container.innerHTML = `
                    <div class="py-12 text-center text-slate-400 dark:text-slate-500 border border-dashed border-slate-200 dark:border-darkBorder rounded-xl">
                        <i class="fa-solid fa-folder-open text-2xl mb-2 block"></i>
                        <p class="text-xs font-bold">No Syllabus Chapters Listed</p>
                        <p class="text-[10px] mt-0.5">Please add dynamic chapters using the right-side controller.</p>
                    </div>
                `;
                    ...
                    <div class="py-3.5 flex items-center justify-between gap-4">
                        <div class="flex items-center gap-3">
                            <button onclick="toggleChapterCompletion(${idx})" class="h-5 w-5 rounded-md border flex items-center justify-center transition-all ${
                                isCompleted 
                                ? 'text-white' 
                                : 'border-slate-300 dark:border-darkBorder bg-transparent'
                            ...
                            <div>
                                ...
                                <span class="text-[9px] text-slate-400 font-semibold">${ch.subtasks} checkpoints configured</span>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="text-[9px] font-bold uppercase tracking-wider" style="color: ${isCompleted ? activeSub.color : '#94a3b8'}">${isCompleted ? 'Done' : 'Active'}</span>
        ...
        function renderCalendar() {
            ...
            for (let i = 0; i < firstDay; i++) {
                grid.innerHTML += `<div class="p-3 bg-slate-50/40 dark:bg-slate-800/10 rounded-xl border border-transparent"></div>`;
            }
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                let borderAccent = 'border-slate-100 dark:border-darkBorder hover:border-indigo-500';
                let indicatorHTML = '';
                ...
                if (matchExams.length > 0) {
                    ...
                    matchExams.forEach(ex => {
                        indicatorHTML += `
                            <div class="text-[8px] px-1 py-0.5 rounded text-white truncate mt-1 w-full" style="background-color: ${ex.color}">
                                ${ex.title}
                            </div>
                        `;
                    });
                ...
                grid.innerHTML += `
                    <div class="min-h-16 p-2 bg-slate-50 dark:bg-slate-800/40 border ${borderAccent} rounded-xl transition-all flex flex-col justify-between items-start">
                        <span class="${matchExams.length > 0 ? 'text-rose-500' : 'text-slate-500 dark:text-slate-400'} font-bold">${day}</span>
        ...
        function renderExams() {
            ...
            container.innerHTML = '';
            if (APP_STATE.exams.length === 0) {
                container.innerHTML = '<p class="text-xs text-slate-400 italic">No exams recorded. Use the creator module to map upcoming evaluations.</p>';
                return;
            }
                ...
                container.innerHTML += `
                    <div class="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-darkBorder flex items-center justify-between gap-4">
                        <div>
                            ...
                            <h4 class="font-bold dark:text-white mt-1.5">${ex.title}</h4>
                            <span class="text-[10px] text-slate-400 font-medium">Target Mark: ${ex.grade} — Day ${ex.day}</span>
                        </div>
        ...
        function initCharts() {
            ...
            const gridColor = isDark ? '#1e293b' : '#ede9fe';
